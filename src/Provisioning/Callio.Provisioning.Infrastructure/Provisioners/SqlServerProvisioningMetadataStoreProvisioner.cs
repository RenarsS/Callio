...
public class SqlServerProvisioningMetadataStoreProvisioner(IConfiguration configuration) : IProvisioningMetadataStoreProvisioner
{
    private readonly string _connectionString = configuration.GetConnectionString("CallioDb")
        ?? throw new InvalidOperationException("A CallioDb connection string is required for provisioning metadata storage.");
